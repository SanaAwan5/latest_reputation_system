...
if __name__ == '__main__':
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        if helper.params['aggregation_methods'] == config.AGGR_MEAN:
            ...
            is_updated, names, weights, alphas, client_grads = helper.foolsgold_update_new(helper.target_model, updates, reputation_dict)
            vis_agg_weight(helper,names,weights,epoch,vis,adversarial_name_keys)
            vis_fg_alpha(helper,names,alphas,epoch,vis,adversarial_name_keys )
            num_oracle_calls = 1

        elif helper.params['aggregation_methods'] == config.AGGR_KRUM:
            is_updated, names, client_grads = helper.krum_update(helper.target_model, updates,clip=5)
            #vis_agg_weight(helper,names,weights,epoch,vis,adversarial_name_keys)
            #vis_fg_alpha(helper,names,alphas,epoch,vis,adversarial_name_keys )
            #num_oracle_calls = 1
